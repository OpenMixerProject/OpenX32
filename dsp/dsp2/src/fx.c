...
#define PHASEINVERTED_BACK_LR	1
...
void fxDecompositingAndUpmixing(float* inBuf[2], float* outBuf[5], int samples) {
	// this is the first approach on implementing an Stereo to 5.0 upmixing algorithm based on the
...
//		memcpy(&maskAmbient_z[0], &maskAmbient[0], sizeof(maskAmbient));
		...
		// ============================================================================================
		for (int k = 0; k < (UPMIX_WINDOW_LEN/2); k++) {
			// left: panned direct sound + 50% ambient
			outputSignalFrequencyDomain[0][k].re = inputSignalFrequencyDomain[0][k].re * maskLeft[k];// * (maskLeft[k] + maskAmbient[k] * 0.5f);
			outputSignalFrequencyDomain[0][k].im = inputSignalFrequencyDomain[0][k].im * maskLeft[k];// * (maskLeft[k] + maskAmbient[k] * 0.5f);

			// right: panned direct sound + 50% ambient
			outputSignalFrequencyDomain[1][k].re = inputSignalFrequencyDomain[1][k].re * maskRight[k];// * (maskRight[k] + maskAmbient[k] * 0.5f);
			outputSignalFrequencyDomain[1][k].im = inputSignalFrequencyDomain[1][k].im * maskRight[k];// * (maskRight[k] + maskAmbient[k] * 0.5f);
		...
		/*
		for (int i_ch = 0; i_ch < 5; i_ch++) {
		    // 1. Addiere das neue IFFT-Ergebnis auf den bestehenden Puffer
		    // Wichtig: Das neue Signal MUSS hier auch gefenstert sein, oder das Fenster-Scaling muss stimmen!
		    vecvaddf(&upmixOutputBuffer[i_ch][0], &upmixNewBuffer[i_ch][0], &upmixOutputBuffer[i_ch][0], UPMIX_WINDOW_LEN);
		}

		// In Step 7 (nachdem die ersten RX_SAMPLE_COUNT ausgegeben wurden):
		// Schiebe den Puffer für den NÄCHSTEN Durchgang um Hop-Size nach links
		for (int i_ch = 0; i_ch < 5; i_ch++) {
		    memmove(&upmixOutputBuffer[i_ch][0], &upmixOutputBuffer[i_ch][UPMIX_RX_SAMPLE_COUNT], (UPMIX_WINDOW_LEN - UPMIX_RX_SAMPLE_COUNT) * sizeof(float));
		    memset(&upmixOutputBuffer[i_ch][UPMIX_WINDOW_LEN - UPMIX_RX_SAMPLE_COUNT], 0, UPMIX_RX_SAMPLE_COUNT * sizeof(float));
		}
		*/
	...
	// set the output with our read-pointer
	outBuf[0] = &upmixOutputBuffer[0][upmixOutputSampleCounter]; // left
	outBuf[1] = &upmixOutputBuffer[1][upmixOutputSampleCounter]; // right
	outBuf[2] = &upmixOutputBuffer[2][upmixOutputSampleCounter]; // center
	outBuf[3] = &upmixOutputBuffer[3][upmixOutputSampleCounter]; // back-left
	outBuf[4] = &upmixOutputBuffer[4][upmixOutputSampleCounter]; // back-right
