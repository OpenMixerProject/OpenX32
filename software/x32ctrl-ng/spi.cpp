...
int spiConfigureFpga(const char* bitstream_path) {
    ...
    helper->Debug("Connecting to SPI for FPGA...\n");
    ...
    if ((bytes_read = fread(file_buffer, 1, FPGA_FILE_BUFFER_SIZE, bitstream_file)) > 0) {
        if (file_buffer[0] != 0xFF) {
            ...
            helper->Debug("Detected bit-file with header...\n");
        }else{
                helper->Debug("Detected bin-file without header...\n");
    ...
    // now send the data
    helper->Debug("Sending bitstream to FPGA...\n");
    ...
    helper->Debug("Setting PROG_B-Sequence HIGH -> LOW -> HIGH and start upload...\n");
    ...
    while ((bytes_read = fread(tx_buffer, 1, FPGA_BUFFER_SIZE, bitstream_file)) > 0) {
        ...
        if (ret < 0) {
            helper->Log("Error: SPI-transmission failed\n");
            if (bitstream_file) fclose(bitstream_file);
            if (spi_fd >= 0) close(spi_fd);
            free(tx_buffer);
            free(rx_buffer);
...
//    uint8_t spiLsbFirst = 0; // Linux-driver for i.MX25 seems to have problems with this option
    ...
    helper->Debug("Connecting to SPI for DSP1...\n");
    spi_fd[0] = open(SPI_DEVICE_DSP1, O_RDWR);
    if (spi_fd[0] < 0) {
        helper->Log("Error: Could not open SPI-device for DSP1\n");
        return -1;
    }
    if (numStreams == 2) {
        ...
        if (spi_fd[1] < 0) {
            helper->Log("Error: Could not open SPI-device for DSP2\n");
            return -1;
        }
    }
...
//        ioctl(spi_fd[1], SPI_IOC_WR_LSB_FIRST, &spiLsbFirst); // this seems to be ignored by i.MX25 linux-driver
    ...
    for (uint8_t i = 0; i < numStreams; i++) {
        ...
        // now send the data
        helper->Debug("Sending bitstream to DSP...\n");
        totalBytesSent = 0;
        last_progress = -1;
...
bool spiOpenDspConnections() {
    ...
    for (uint8_t i = 0; i < 2; i++) {
        ...
        if (spiDspHandle[i] < 0) {
            helper->Debug("Error: Could not open SPI-device");
            return false;
        }
