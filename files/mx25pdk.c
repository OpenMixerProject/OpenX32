...
static void serial_init(struct mxc_uart *base, int use_dte)
{
	writel(0, &base->cr1);
	writel(0, &base->cr2);
...
static void serial_setbaudrate(struct mxc_uart *base, unsigned long clk,
...
void set_lcd(uint8_t boardId, uint8_t index, uint8_t color) {
	// boardId = 0, 4, 5, 8
	// index = 0 ... 8
...
int board_late_init(void)
{
	...
	serial_init(mxc_uart_base_surface, 0);
	u32 clk = imx_get_uartclk();
	serial_setbaudrate(mxc_uart_base_surface, clk, 115200, false);

	// set first LCD on board 0 and display text "OpenX32! Booting"
	set_lcd(0, 0, 2); // boardId, index
